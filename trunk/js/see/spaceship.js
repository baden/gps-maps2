/**
	...
	Inspire of http://erkie.github.com/
**/


var SCRIPTURL = 'http://m-ms.staging-bbdo.dk/bluecssversion/js/';
var IMAGESURL = 'http://m-ms.staging-bbdo.dk/bluecssversion/images/';
var spaceShipImgSrcs = ['spaceship_11.png', 'spaceship_12.png', 'spaceship_13.png'];
var bulletImgSrcs = ['MMs_tablets_01.png', 'MMs_tablets_02.png', 'MMs_tablets_03.png', 'MMs_tablets_04.png', 'MMs_tablets_05.png', 'MMs_tablets_06.png'];
...
function Invaders() {
	...
	*/
	...
	var playerWidth = 120 + 23, playerHeight = 100;
	...
	for(var bi = 0; bi < bulletImgSrcs.length; bi++){
		var bulletImg = new Image();
		bulletImg.src = IMAGESURL + bulletImgSrcs[bi];
	...
	this.toggleBlinkStyle = function () {
		if (this.updated.blink.isActive) {
			removeClass(document.body, 'INVADERSBLINK');
		} else {
			addClass(document.body, 'INVADERSBLINK');
		}

		this.updated.blink.isActive = !this.updated.blink.isActive;
	};

	addStylesheet(".INVADERSBLINK .INVADERSYEAHENEMY", "outline: 2px dotted red;");
	...
	*/
	...
	function applyVisibility(vis) {
		for ( var i = 0, p; p = window.INVADERSPLAYERS[i]; i++ ) {
	...
	function setScore() {
		that.points.innerHTML = window.INVADERS.enemiesKilled * 10;
	};
	...
	function addStylesheet(selector, rules) {
		...
		stylesheet.id = 'INVADERSYEAHSTYLES';
		try {
	...
	*/
	...
	this.gameContainer.className = 'INVADERSYEAH';
	document.body.appendChild(this.gameContainer);
	
	this.canvas = document.createElement('canvas');
	this.canvas.setAttribute('width', w);
	this.canvas.setAttribute('height', h);
	this.canvas.className = 'INVADERSYEAH';
	...
	if ( ! document.getElementById('INVADERS-NAVIGATION') ) {
		this.navigation = document.createElement('div');
		this.navigation.id = "INVADERS-NAVIGATION";
		this.navigation.className = "INVADERSYEAH";
		...
		this.points.id = 'INVADERS-POINTS';
		this.points.style.font = "28pt Arial, sans-serif";
		this.points.style.fontWeight = "bold";
		this.points.className = "INVADERSYEAH";
		this.navigation.appendChild(this.points);
	} else {
		this.navigation = document.getElementById('INVADERS-NAVIGATION');
		this.points = document.getElementById('INVADERS-POINTS');
	}
	...
	*/
	...
	this.ctx.drawBullets = function(bullets) {
		for ( var i = 0; i < bullets.length; i++ ) {
			this.drawImage(bulletImgs[bullets[i].type], bullets[i].pos.x - halfBulletW, bullets[i].pos.y - halfBulletH);
		}
	};
	...
	*/
	...
	addClass(document.body, 'INVADERSYEAH');
	...
	this.update = function() {
		...
		if ( this.keysPressed[code(' ')] && nowTime - this.firedAt > timeBetweenFire ) {
			this.bullets.unshift({
				'dir': this.dir.cp(),
				'pos': this.pos.cp().add(new Vector(0, 35).setAngle(this.dir.angle())),
				//'pos': this.pos.cp(),
		...
		if ( this.keysPressed[code('esc')] ) {
			...
			this.keysPressed[code('esc')] = false; //fixed, FPS may to high, so setTimeout may cause bug here
		...
		for ( var i = this.bullets.length - 1; i >= 0; i-- ) {
			...
			if (
				...
				hasOnlyTextualChildren(murdered) && murdered.className != "INVADERSYEAH"
			) {
				didKill = true;
	...
	function destroy() {
		...
		isRunning = false;
		removeStylesheet("INVADERSYEAHSTYLES");
		removeClass(document.body, 'INVADERSYEAH');
		this.gameContainer.parentNode.removeChild(this.gameContainer);
		
		window.INVADERSPLAYERS.length = 0; //fix single instance bug
	};
}

if (!window.INVADERSPLAYERS || window.INVADERSPLAYERS.length == 0 ){ //block to only one instance of Invader

	window.INVADERSPLAYERS = [];
	...
	if ( window.ActiveXObject && ! document.createElement('canvas').getContext ) {
		...
		script.onreadystatechange = function() {
			if ( script.readyState == 'loaded' || script.readyState == 'complete' ) {
				if ( typeof G_vmlCanvasManager != 'undefined' )
					window.INVADERSPLAYERS[window.INVADERSPLAYERS.length] = new Invaders();
			}
		};
		script.src = SCRIPTURL + 'excanvas.js';
		document.getElementsByTagName('head')[0].appendChild(script);
	}
	else window.INVADERSPLAYERS[window.INVADERSPLAYERS.length] = new Invaders();
}
